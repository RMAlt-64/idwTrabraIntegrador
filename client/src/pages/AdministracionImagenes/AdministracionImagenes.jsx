...
const AdministracionImagenes = () => {
    const [imagenes, setImagenes] = useState([]);
    const [alojamientos, setAlojamientos] = useState([]); // Para almacenar los alojamientos
    const [loading, setLoading] = useState(true);
    const [showModal, setShowModal] = useState(false);
    const [currentImagen, setCurrentImagen] = useState({ idAlojamiento: '', RutaArchivo: '' });
    const [modalMode, setModalMode] = useState('');
    ...
    const handleCreate = () => {
        setCurrentImagen({ idAlojamiento: '', RutaArchivo: '' });
        setModalMode('create');
        setShowModal(true);
    };
                    ...
                    {imagenes.map((imagen, index) => (
                        <tr key={index}>
                            ...
                            <td className='text-center align-middle'>
                                {alojamientos.find(a => a.idAlojamiento === imagen.idAlojamiento)?.Titulo}
                            </td>
                            <td className='text-center align-middle'>
                                <img src={imagen.RutaArchivo} alt={`Imagen de ${imagen.idAlojamiento}`} style={{ maxWidth: '100px' }} />
                            </td>
                            <td>
                                <div className='container-fluid d-flex justify-content-center align-items-center'>
                                    <Button variant="warning" size="sm" className="me-2" onClick={() => handleEdit(imagen)}>Modificar</Button>
                        ...
                        <Form>
                            <Form.Group controlId="formAlojamiento">
                                ...
                                <Form.Control
                                    as="select"
                                    name="idAlojamiento"
                                    value={currentImagen.idAlojamiento}
                                    onChange={(e) => setCurrentImagen({ ...currentImagen, idAlojamiento: e.target.value })}
                                >
                            ...
                            <Form.Group controlId="formRutaArchivo">
                                <Form.Label>URL de la Imagen</Form.Label>
                                <Form.Control
                                    type="text"
                                    name="RutaArchivo"
                                    value={currentImagen.RutaArchivo}
                                    onChange={(e) => setCurrentImagen({ ...currentImagen, RutaArchivo: e.target.value })}
                                />
                            </Form.Group>
                        </Form>
                    </Modal.Body>
